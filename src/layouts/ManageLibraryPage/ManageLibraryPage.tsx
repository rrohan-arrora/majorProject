...
export const ManageLibraryPage = () => {
    ...
    const [changeQuantityBooks, setChangeQuantityBooks] = useState(false);
    const [checkMessages, setCheckMessages] = useState(false);

    function addBook() {
        setChangeQuantityBooks(false);
        setCheckMessages(false);
    }

    const changeQuantityofBooks = () => {
        setChangeQuantityBooks(true);
        setCheckMessages(false);
    }
            ...
            <div className="mt-5">
                ...
                <div className="tab-content" id="nav-tabContent">
                    ...
                    aria-labelledby="nav-quantity-tab">
                        {changeQuantityBooks ? <ChangeQuantityofBooks/> : <></>}
                    </div>
                    <div className="tab-pane fade " id="nav-messages" role="tabpanel"
                    aria-labelledby="nav-messages-tab">
                        {checkMessages ? <AdminMessages/> :<></>}
                    </div>
                </div>
            </div>
        </div>
